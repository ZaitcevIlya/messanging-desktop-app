...
class RegisterUser(QDialog):
    ...
    def __init__(self, database, server):
        ...
        self.btn_ok = QPushButton('Create', self)
    ...
    def save_data(self):
        """
        Validate and save new user to DB
        """
        if not self.client_name.text():
            self.messages.critical(
                self, 'Error', 'You must enter username.')
            return
        elif self.client_passwd.text() != self.client_conf.text():
            self.messages.critical(
                self, 'Error', 'Entered passwords do not match')
            return
        elif self.database.check_user(self.client_name.text()):
            self.messages.critical(
                self, 'Error', 'Such user already exists')
            return
        else:
